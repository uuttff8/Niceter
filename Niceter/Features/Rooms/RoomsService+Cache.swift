    ...
    func fetchData(then handler: @escaping Handler) {
        self.storage?.async.object(forKey: self.cacheKey, completion: { (res) in
            ...
            case .error(let error):
                NiceterLog.logCacheError(title: "Failed to prefetch suggested room cache", error: error)
            }
        })
        ...
        self.storage?.async.object(forKey: self.cacheKey, completion: { result in
            switch result {
              case .value(let rooms):
                handler(rooms)
              case .error(let error):
                NiceterLog.logCacheError(title: "Failed to fetch room loader cache", error: error)
                break
            }
        })
        ...
        self.storage?.async.object(forKey: self.cacheKey, completion: { result in
            switch result {
              case .value(let users):
                handler(users)
              case .error(let error):
                NiceterLog.logCacheError(title: "Failed to prefetch room users cache", error: error)
            }
        })
